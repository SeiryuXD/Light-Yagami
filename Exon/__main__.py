...
buttons = [
    ...
    [
        InlineKeyboardButton(text="ᴄʟɪᴄᴋ ʜᴇʀᴇ ғᴏʀ ʜᴇʟᴘs", callback_data="help_back"),
        InlineKeyboardButton(text="sᴛᴀᴛs", callback_data="stats_callback"),
    ],
    [
        InlineKeyboardButton(text="ᴀʙᴏᴜᴛ ᴛʜɪs ʙᴏᴛ", callback_data="ABG_"),
        InlineKeyboardButton(
...
def start(update: Update, context: CallbackContext):    # sourcery no-metrics
    ...
    if update.effective_chat.type == "private":
        if args and len(args) >= 1:
            ...
            lol.edit_text("⚡")
            time.sleep(0.3)
            lol.edit_text("ꜱᴛᴀʀᴛɪɴɢ ᴛʜᴇ sʏsᴛᴇᴍs... ")
            time.sleep(0.4)
            lol.delete()
